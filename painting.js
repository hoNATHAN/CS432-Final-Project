class Painting extends Drawable {
    ...
    static generatePainting() {
        ...
        Painting.vertexTextureCoords = [
            // Front face
            vec2(0, 1), vec2(1, 1), vec2(1, 0), vec2(0, 0),  // Flipping Y for front
            // Back face
            vec2(0, 1), vec2(1, 1), vec2(1, 0), vec2(0, 0),  // Flipping Y for back
          ];
    ...
    static initialize() {
      ...
      gl.bufferData(gl.ARRAY_BUFFER, flatten(Painting.vertexPositions), gl.STATIC_DRAW);
  
      Painting.textureCoordBuffer = gl.createBuffer();
      gl.bindBuffer(gl.ARRAY_BUFFER, Painting.textureCoordBuffer);
      gl.bufferData(gl.ARRAY_BUFFER, flatten(Painting.vertexTextureCoords), gl.STATIC_DRAW);
  
      Painting.indexBuffer = gl.createBuffer();
      gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, Painting.indexBuffer);
      gl.bufferData(gl.ELEMENT_ARRAY_BUFFER, new Uint16Array(Painting.indices), gl.STATIC_DRAW);
  
      Painting.aPositionShader = gl.getAttribLocation(Painting.shaderProgram, "aPosition");
      Painting.aTextureCoordShader = gl.getAttribLocation(Painting.shaderProgram, "aTextureCoord");
      Painting.uModelMatrixShader = gl.getUniformLocation(Painting.shaderProgram, "modelMatrix");
      Painting.uCameraMatrixShader = gl.getUniformLocation(Painting.shaderProgram, "cameraMatrix");
      Painting.uProjectionMatrixShader = gl.getUniformLocation(Painting.shaderProgram, "projectionMatrix");
      Painting.uTextureUnitShader = gl.getUniformLocation(Painting.shaderProgram, "uTextureUnit");
    }
    ...
    draw() {
      ...
      gl.bindBuffer(gl.ARRAY_BUFFER, Painting.textureCoordBuffer);
      gl.vertexAttribPointer(Painting.aTextureCoordShader, 2, gl.FLOAT, false, 0, 0);
      ...
      gl.uniformMatrix4fv(Painting.uModelMatrixShader, false, flatten(this.modelMatrix));
      gl.uniformMatrix4fv(Painting.uCameraMatrixShader, false, flatten(camera.cameraMatrix));
      gl.uniformMatrix4fv(Painting.uProjectionMatrixShader, false, flatten(camera.projectionMatrix));
  
      gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, Painting.indexBuffer);
      gl.drawElements(gl.TRIANGLES, Painting.indices.length, gl.UNSIGNED_SHORT, 0);
